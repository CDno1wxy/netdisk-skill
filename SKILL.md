...
- TMDB 识别：需要 TMDB API Key（`--api-key`、环境变量 `ENV_TMDB_API_KEY` 或 `~/.tmdb-api-key` 文件）；可选配置 Tavily API Key（环境变量 `TAVILY_API_KEY` 或 `~/.tavily-api-key`），先搜索官方片名再交给 TMDB 校验。
...
扫描默认的 `云下载` 和 `转存`，先用目录名、失败后再用实际视频文件名调用 TMDB，按 `整理/电影`、`整理/剧集`、`整理/动漫`、`整理/纪录片` 下的 `{tmdb-ID}` 目录匹配。默认只演练，确认结果后加 `--apply` 执行；会将匹配目录下的文件和子目录整体移动到目标目录，保留视频、字幕、NFO、图片及发布页文件，不删除任何文件，只清理移动后留下的空源目录。未识别或未找到对应目录的项目会保留原位。
