...
提交磁力时会把磁力哈希、原始名称、片名和年份保存到 `~/.codex/netdisk-download-metadata.json`；如果 TMDB 能识别，还会一并保存 TMDB ID、媒体类型和分类。整理时优先使用这份提交记录，避免仅根据下载目录名重新猜测。
...
扫描默认的 `云下载` 和 `转存`，优先使用提交磁力时保存的元数据，再用目录名、失败后再用实际视频文件名调用 TMDB，按 `整理/电影`、`整理/剧集`、`整理/动漫`、`整理/纪录片` 下的 `{tmdb-ID}` 目录匹配。默认只演练，确认结果后加 `--apply` 执行；会将匹配目录下的文件和子目录整体移动到目标目录，保留视频、字幕、NFO、图片及发布页文件，不删除任何文件，只清理移动后留下的空源目录。未识别或未找到对应目录的项目会保留原位。
