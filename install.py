#!/usr/bin/env python3
"""
Install the netdisk (115/123) Codex skill from a GitHub repository or local checkout.
...
def download_archive(owner: str, repo: str, ref: str, archive_url: Optional[str]) -> bytes:
    ...
    request = urllib.request.Request(
        archive_url,
        headers={"User-Agent": "netdisk-skill-installer/1.0"},
    )
...
def verify_install(skill_dir: Path) -> None:
    required = [
        ...
        skill_dir / "scripts" / "netdisk.py",
        skill_dir / "scripts" / "transfer_share.py",
    ]
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="netdisk-skill-install-") as tmp:
