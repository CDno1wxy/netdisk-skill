...
def new_115_client(cookies_or_path):
    """
    Create a P115Client with version compatibility.

    Newer p115client releases dropped the `check_for_relogin` kwarg; older
    releases may not accept PathLike cookies. Try a couple of signatures.
    """
    P115Client = import_p115client()
    try:
        return P115Client(cookies_or_path, check_for_relogin=True)
    except TypeError:
        pass
    try:
        return P115Client(cookies_or_path)
    except TypeError:
        return P115Client(cookies=str(cookies_or_path))
...
def patch_p115client_default_parse() -> None:
    ...
    try:
        ...
        return
    default_parse = getattr(client_mod, "default_parse", None)
    if default_parse is None:
        # 新版 p115client 已无此属性，无需打补丁
        return
    if getattr(default_parse, "_p115_skill_patched", False):
        return
...
def get_client_from_cookies(cookies: str):
    ...
    P115Client = import_p115client()
    return configure_client(new_115_client(cookies), cookies)
...
def list_115_files_app(client, cid=0, limit=50, offset=0):
    """
    列出 115 目录，兼容不同 p115client 版本。

    新版 p115client 的 fs_files_aps / fs_files 接口已失效（HTTP 405），
    优先使用 fs_files_app。
    """
    for method in ("fs_files_app", "fs_files_aps"):
        fn = getattr(client, method, None)
        if not callable(fn):
            continue
        try:
            return fn({"cid": cid, "limit": limit, "offset": offset})
        except Exception:
            continue
    raise RuntimeError("当前 p115client 无可用文件列表接口")


def print_item(item: dict):
    """格式化打印一个文件/文件夹条目（兼容 fs_files_aps 与 fs_files_app 两种格式）。"""
    name = item.get('fn') or item.get('n') or item.get('name') or item.get('file_name') or '?'
    fid = item.get('fid') or item.get('cid') or item.get('id')
    has_app_shape = 'fn' in item and 'fid' in item and 's' not in item and 'size' not in item
    is_dir = item.get('cid') is not None or item.get('fc') is not None or has_app_shape
    suffix = f" (ID: {fid})" if fid else ""
    if is_dir:  # 文件夹
        print(f"  📁 {name}/ ({item.get('fc', 0)} 项){suffix}")
    else:  # 文件
        size = format_size(item.get('s', item.get('size', 0)))
        print(f"  📄 {name} ({size}){suffix}")
